puts "Loading [1110_create_customers] file..."
...
File.open(File.dirname(__FILE__) + '/CUSTOMERS.csv', 'r').each_with_index do |line, index|
  if index == 0
    # 첫 번째 라인은 헤더 정보 
    ...
    save_data(domain.customers, data, admin)
    data_count += 1
  end
end

puts "Total #{data_count} Customers Loaded!"