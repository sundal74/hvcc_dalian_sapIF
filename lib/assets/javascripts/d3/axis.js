    ...
    selection.each(function(data) {
      ...
      switch (axis.orient()) {
        ...
        case 'bottom':
          var xLabelMargin = 36;
          var maxTextWidth = 30;
          var xLines = g.selectAll('g').select("line");
          xLines.each(function(d,i){
          	var dStr = d+"";
	        if (dStr.length > 0){
		      	for (j=0; j<dStr.length; j++){
		             if (((dStr.substring(j, j+1) < "0") || (dStr.substring(j, j+1) > "9"))){
...
//              .attr('text-anchor', rotateLabels%360 > 0 ? 'start' : 'end');
            xTicks
              .attr('transform', function(d,i,j) { return 'rotate(' + rotateLabels + ' 0,10)' })
              .attr('text-anchor', rotateLabels%360 > 0 ? 'start' : 'end');
          }
          axisLabel.enter().append('text').attr('class', 'nv-axislabel')
            .attr('text-anchor', 'middle')
