puts "Loading [1060_create_boms] file..."

ProductPart.delete_all
separator = ','
...
File.open(File.dirname(__FILE__) + '/BOMS.csv', 'r').each_with_index do |line, index|
  if index == 0
    # 첫 번째 라인은 헤더 정보 
    ...
    save_data(domain.product_parts, data, admin)
    data_count += 1
  end
end

puts "Total #{data_count} BOM Loaded!"