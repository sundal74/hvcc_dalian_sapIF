...
Ext.define('Ext.ux.DateTimePicker', {
	  ...
	  onRender: function(container, position) {
		  ...
		  // this.okBtn = new Ext.Button('Ext.Button', {
		  // 			  text: "ok",
		  // 			  width: 30,
		  // 			  height: 30
		  // });
		  // 
		  // this.okBtn.on('click', this.selectOk, this);
		  ...
		  // this.okBtn.render(this.el.child('div div.x-datepicker-footer'));
	  ...
	  handleDateClick: function(e, t) {
		  ...
		  if(!me.disabled && t.dateValue && !Ext.fly(t.parentNode).hasCls(me.disabledCellCls)) {
			  ...
			  me.setValue(new Date(t.dateValue)); // overwrite: fill time before setValue
	  ...
	  selectToday: function() {
		  ...
		  if(btn && !btn.disabled) {
			  ...
			  me.setValue(this.fillDateTime(new Date(me.getValue())));// overwrite: fill time before setValue
			  me.fireEvent('select', me, me.value);
			  if(handler) {
				  handler.call(me.scope || me, me, me.value);
			  }
