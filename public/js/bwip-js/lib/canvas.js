...
(function() {
	...
	for (var i = 0; i < symdesc.length; i++) {
		if (!symdesc[i])	// last entry can be null
			continue;
		emit('js/bwip-js/bwipp/' + symdesc[i].sym);
	}
	*/

	// Renderers
	emit('js/bwip-js/bwipp/renlinear');
	emit('js/bwip-js/bwipp/renmatrix');
	emit('js/bwip-js/bwipp/renmaximatrix');

	// Fonts
	for (var x = 1; x <= 10; x++) {
		emit('js/bwip-js/fonts/ocrb10-' + (x < 10 ? '0' : '') + x);
		emit('js/bwip-js/fonts/ocrb12-' + (x < 10 ? '0' : '') + x);
	}
})();
